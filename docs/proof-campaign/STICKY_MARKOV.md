# Entropy profile: Sticky d6

EntropyLab profiles an entropy-generation process. It does not generate, inspect, or handle seed material.

## Result

**0.4765 bits per symbol** (conservative estimate, against an ideal of 2.5850 for a 6-symbol alphabet).
...
- The estimators disagree by 2.11 bits per symbol. Disagreement is expected when one method detects structure the others cannot see, and the lowest figure governs.
...
- **Collision:** This estimate is computed over a 3-bit encoding of a 6-symbol alphabet. Because 6 is not a power of two, unreachable codewords bias the bitstring (observed one-rate 0.3953) independently of the source. The figure describes the source and this encoding together, not the source alone.
