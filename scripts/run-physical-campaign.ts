...
/**
 * Observations the protocol calls for in each session.
 *
 * A session that does not match is reported rather than silently accepted, so
 * a miscount is caught before it reaches a published figure. Set the
 * EXPECTED_PER_SESSION environment variable to override for a session that was
 * deliberately a different length.
 */
const EXPECTED_PER_SESSION = Number(process.env.EXPECTED_PER_SESSION ?? 256);
...
function readSessions(): Session[] {
  ...
  return files
    ...
    .map((file) => {
      ...
      if (
        Number.isFinite(EXPECTED_PER_SESSION) &&
        sample.observations.length !== EXPECTED_PER_SESSION
      ) {
        process.stderr.write(
          `${file}: ${sample.observations.length} observations, protocol expects ` +
            `${EXPECTED_PER_SESSION}. Set EXPECTED_PER_SESSION to override.\n`,
        );
        process.exit(1);
      }
...
function main(): void {
  ...
  const doc = [
    ...
    "",
    `**This is a practical calibration demonstration using ${combinedSample.observations.length}`,
    "physical observations. It is not formal entropy-source certification.**",
    "",
    "SP 800-90B's assessment process assumes at least 1,000,000 samples from a",
    `documented noise source. These are ${combinedSample.observations.length} rolls recorded by hand. They support a`,
    "decision about whether to trust one procedure, made by the person who",
    "performed it. They do not support a certification, and none is claimed.",
    "",
    ...
    "",
    "Per-session and combined figures answer different questions and are not merged.",
    "Per session shows whether the process stayed the same; at this sample size those",
    "figures are individually weak and some estimators will mark themselves unstable.",
    "Combined is the better measurement across all observations, and the worse drift",
    "detector.",
    "",
    "Provenance: **physical calibration sample**. Rolled by hand and recorded in",
    "order. Every other dataset in this repository is a synthetic adversarial",
    "fixture and is labelled as such; the two are never compared without saying so.",
    "",
    "| Field | Value |",
    "| --- | --- |",
    `| Source type | ordinary six-sided die |`,
    `| Sessions | ${sessions.length} |`,
    `| Observations per session | ${sessions.map((s) => s.sample.observations.length).join(", ")} |`,
    `| Total observations | ${combinedSample.observations.length} |`,
    `| Algorithm version | ${combinedAnalysis.version} |`,
    `| Report format | ${combinedReport.reportFormatVersion} |`,
    "",
    "### Input hashes",
    "",
    "SHA-256 over the canonical dataset, computed before analysis.",
    "",
    "| Dataset | Hash |",
    "| --- | --- |",
    ...sessions.map((s) => `| ${s.id} | \`${hashSample(s.sample)}\` |`),
    `| combined | \`${hashSample(combinedSample)}\` |`,
    "",
    "### Results",
    "",
